...
public:
    ...
    void Go(int num_slits, Movement movement, int speed_perc)
    {
        ...
        else if (movement == MOVE_LEFT)
        {
            ...
            current_degrees_ = current_degrees_ - 90;
            if (current_degrees_ < 0) {
                current_degrees_ += 360;
            }
    ...
    void loop()
    {
        ...
        if (now > last_time_ + kCheckTime && is_moving()) {
            last_time_ = now;
            //for (int i = 0; i < NUM_MOTORS; i++)
            //{
