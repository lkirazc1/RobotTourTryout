...
public:
    ...
    bool stop_motor_turning(double direction_diff)
    {
        ...
        {
            return direction_diff < 3;
        }
        return direction_diff < 2;
    }
    ...
    void loop()
    {
        ...
        if (now > last_time_p_ + kCheckTimeP && is_moving() && (movement == MOVE_RIGHT || movement == MOVE_LEFT))
        {
            ...
            if (movement == MOVE_LEFT)
            {
                new_speed = (int)((abs(current_degrees_ - angularPos) - 3) * kp_left + (now - time_at_last_go) * ki_left);
            }
            else
            {
                new_speed = (int)((abs(current_degrees_ - angularPos) - 2) * kp_right + (now - time_at_last_go) * ki_right);
            }
...
private:
    ...
    const double ki_left = 0.1;
    const double kp_right = 7;
    const double ki_right = 0.1;
    const double left_buffer = 3;
    const double right_buffer = 2;
