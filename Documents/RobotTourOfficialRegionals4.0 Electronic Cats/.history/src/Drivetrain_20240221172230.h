...
public:
    ...
    void loop()
    {
        ...
        if (now > last_time_p_ + kCheckTimeP && is_moving() && (movement == MOVE_RIGHT || movement == MOVE_LEFT))
        {
            ...
            {
                new_speed = (int)((abs(current_degrees_ - angularPos) - 1) * kp_right + (now - time_at_last_go) * ki_right);
            }
...
private:
    ...
    const double kp_left = 7;
    const double ki_left = 0.2;
    const double kp_right = 7;
    const double ki_right = 0.02;
