...
public:
    ...
    void loop()
    {
        ...
        if (now > last_time_ + kCheckTime && is_moving() && (movement == MOVE_LEFT || movement == MOVE_RIGHT)) {
            ...
            angularPos -= current_z**.001; //integrates angular velocity with respect to time
            if (angularPos < 0) {kCheckTime
                angularPos += 360;
            }
            if (angularPos > 360) {
                angularPos -= 360;
...
private:
    ...
    const double z_offset = 1.101;
    const double kp_left = 2;
    const double ki_left = 0.02;
    const double kp_right = 4;
    const double ki_right = 0.04;
